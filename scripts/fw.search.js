...
(function() {
    var $form = $('.searchform form'),
        ...
        $suggest = $('.searchform__suggests'),
        $message = $suggest.find('.message'),
    ...
    function getSuggest( searchvalue ) {
        $.getJSON( "zoeken.json?search=" + searchvalue, function( data ) {
            var items = [];
            ...
            if( data.length > 0 ) {
                ...
                $('.searchform').find('ul').html( items.join('') );
                showSuggest();
